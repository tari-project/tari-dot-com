...
The Tari development community is hard at work on the Layer 2 [DAN], and this week we take a look at the Covenants RFC and some quality of life improvements in the Tari mobile wallet [Aurora].
...
Let's take a quick look at the proposal. Covenants allow the spender of a transaction output ([UTXO]) to place restrictions on how that output can subsequently be spent. The proposed implementation will add a `covenant` field to each transaction input and output. Normal transactions will have the identity (ie. empty) covenant - meaning that no restriction is applied.
...
The RFC further describes how covenants will be implemented as [set operations], filtering the output set using a discreet set of rules. The covenant passes if at least one output in the set is matched after the filtering rules are applied.

More information on the proposed rules and other examples are available [in the RFC].
...
That means a lot less typing since 2 to 4 letters are enough to find your word and tap the suggestion 😎
