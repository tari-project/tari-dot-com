...
The primary focus this quarter is squarely on preparing for mainnet audit readiness. However, work on the contract 
layer has not stopped, and we thought it an opportune time to 
share a glimpse into what has been happening on the Tari Digital Asset Network (DAN) layer. 
...
* Running a small network of 2 validator nodes that:
  ...
  * reach consensus on rejecting an invalid instruction.

You can, with a bit of elbow grease, carry out this exercise yourself using the code in 
[DAN repo](https://github.com/tari-project/tari-dan).

## The contract template

If you've been following the project for a while, you'll know that it's always been Tari's goal to improve the 
overall safety of the ecosystem by allowing users to draw from a library of well-tested, reliable, 
performance contract _templates_.

These templates are written in Rust, and look something like this:
...
Little bits of code like the snippet above is nowhere near sufficient to work effectively in a decentralised, 
permissionless smart-contract network. There is a huge amount of infrastructure and glue code that must be packed 
around the template above to let it slot seamlessly into the DAN. Suffice to say, the innocuous little  
...
The WASM builder leverages the amazing work in WASM interoperability done by the Rust community. 
...
If you pry into the generated code, you'll find our contract template in `package\src\lib.rs`. 
...
[RFC-0303](https://rfc.tari.com/RFC-0303_DanOverview.html#the-tari-base-layer), all templates are registered on the 
base layer chain so that all nodes have a global index of the set of templates that are available.
...
* The template code wash hashed, to obtain the unique, permanent id of the template.
* The validator node connected with a Tari wallet to perform a registration transaction, and provided the requisite 
  metadata to complete the transaction.
* The code itself (95kB) is not stored on-chain; just the hash, so we need a method of 
...
Once the transaction is broadcast, it shows up like any other transaction:
...
Binary Interface (ABI), or in other words, a menu of the functions that the template exposes, 
...
The other output is related to some technical aspects of the Cerberus consensus algorithm and isn't that important 
to this story.

## Invoking contract methods

Now that we have a live contract on our 2-node DAN (good name for a punk rock band!), we can _do_ things with it! 
