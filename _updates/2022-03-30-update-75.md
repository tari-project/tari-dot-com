...
Consensus Encoding is a strictly defined method of serializing and deserializing the data structures sent over the network.
This ensures that there is no ambiguity in field ordering or encoding, which is important since an item's hash will not match if there are any differences.

The previous header hashing method relied on [Protocol Buffers] for the final serialization, which could change across versions.
[#3820] updated the header hashing method to use consensus encoding, which serves as a formal specification and reusable component for other implementations to use.
