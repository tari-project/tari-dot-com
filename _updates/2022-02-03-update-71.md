...
full block is received, it is validated and the block hash re-propagated to any peers (excluding the original sending peer) so that they
too can be made aware of the new block.
...
- list of transaction kernel excess signatures. 
...
only the missing transactions are requested from the peer and added to the block. In most cases, this won't be necessary.
