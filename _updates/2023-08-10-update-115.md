...
As you may be aware, we have a regular network rollover every 8 weeks. This week is the designated rollover period, which means that Nextnet will transition to Stagenet, and Development will move to Nextnet. With these transitions, it's important to note that each network will undergo a reset. Therefore, we kindly remind you to clean up your network folders accordingly.

The primary focus of these changes is centered around bolstering security measures for Tari, ensuring a robust and bug-free environment before our official launch. Our team has been diligent in conducting an internal audit, addressing potential vulnerabilities, and closing any identified loopholes. Concurrently, we are engaging in an external audit to further enhance the security of our network.
...
During this period, we have prioritized a rigorous internal audit and devoted significant resources to address security concerns. We've diligently tackled a wide range of bugs, addressing issues from refining the mempool's handling of zero-confirmation transactions to resolving malleability bugs present in the headers.

A notable improvement in our wallet recovery process is worth highlighting. We've transitioned away from the practice of rewinding the bulletproof, opting for a more streamlined approach: now, the blinding factor and value on the utxo are simply encrypted, enhancing the efficiency of the recovery mechanism.

Additionally, we've introduced a noteworthy enhancement to the Tari layer - the incorporation of a chat FFI client tailored specifically for peer-to-peer (p2p) chat messaging. This addition significantly enhances the communication capabilities within the Tari network, enriching the overall user experience.
