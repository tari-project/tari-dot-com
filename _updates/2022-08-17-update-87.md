...
In keeping with the tiding of the repository, we have removed all migration code added to ensure the local storage does not break when updating nodes on Dibbler. This includes taking time to reduce the database migration code to only a single SQL up file. 
...
### Burning of outputs
Burning of coins has been a long-used technique to accomplish a multitude of things in crypto, typically done with a “broken unclaimable script/key”. We have added an explicit burn output in which a specific output can be burned and thus never be used again. The future intention is to use burning to get layer two funds. 
